    ...
    namespace crypto3 {
        namespace multiprecision {
            namespace cpp_bf_io_detail {
                ...
                //
                template<class I>
                inline I restricted_multiply(cpp_int& result, const cpp_int& a, const cpp_int& b, I max_bits,
                                             std::int64_t& error) {
                    result = a * b;
                    I gb = msb(result);
                    I rshift = 0;
                ...
                inline int get_round_mode(const cpp_int& what, std::int64_t location, std::int64_t error) {
                    ...
                    if (bit_test(what, static_cast<unsigned>(location))) {
                        if ((int)lsb(what) == location)
                            return error ? -1 :
                                           1;    // Either a tie or can't round depending on whether we have any error
                        if (!error)
            ...
            namespace backends {
                ...
                template<unsigned Digits, digit_base_type DigitBase, class Allocator, class Exponent, Exponent MinE,
                         Exponent MaxE>
                cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>&
                    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::operator=(const char* s) {
                    cpp_int n;
                    std::intmax_t decimal_exp = 0;
                    std::intmax_t digits_seen = 0;
                    constexpr const std::intmax_t max_digits_seen =
                        4 +
                        (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count * 301L) / 1000;
                    bool ss = false;
                    //
                    // Extract the sign:
                    //
                    ...
                    if ((std::strcmp(s, "nan") == 0) || (std::strcmp(s, "NaN") == 0) || (std::strcmp(s, "NAN") == 0)) {
                        return *this = std::numeric_limits<number<
                                   cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::quiet_NaN()
                                           .backend();
                    }
                    if ((std::strcmp(s, "inf") == 0) || (std::strcmp(s, "Inf") == 0) || (std::strcmp(s, "INF") == 0) ||
                        (std::strcmp(s, "infinity") == 0) || (std::strcmp(s, "Infinity") == 0) ||
                        (std::strcmp(s, "INFINITY") == 0)) {
                        *this =
                            std::numeric_limits<
                    ...
                    if (*s) {
                        ...
                        BOOST_THROW_EXCEPTION(
                            std::runtime_error("Unable to parse string as a valid floating point number."));
                    }
                    if (n == 0) {
                    ...
                    // a value as small as possible > cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                    // MaxE>::bit_count to avoid large computations and excessive memory usage, but we also want to
                    // avoid having to up the computation and start again at a higher precision. So we round
                    // cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count up to the nearest
                    // whole number of limbs, and add one limb for good measure.  This works very well for small
                    // exponents, but for larger exponents we may may need to restart, we could add some extra precision
                    // right from the start for larger exponents, but this seems to be slightly slower in the *average*
                    // case:
                    //
#ifdef BOOST_MP_STRESS_IO
                    std::intmax_t max_bits =
                        cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + 32;
#else
                    std::intmax_t max_bits =
                        ...
                        ((cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count % limb_bits) ?
                             (limb_bits - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count %
                                              limb_bits) :
                             0) +
                        limb_bits;
#endif
                    ...
                    if (decimal_exp >= 0) {
                        ...
                        do {
                            cpp_int t;
                            if (decimal_exp) {
                                calc_exp = nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_pow(
                                    t, cpp_int(5), decimal_exp, max_bits, error);
                                calc_exp += nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_multiply(
                                    t, t, n, max_bits, error);
                            } else
                                t = n;
                            final_exponent =
                                (std::int64_t)
                                    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count -
                                1 + decimal_exp + calc_exp;
                            int rshift = msb(t) -
                                         cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count +
                                         1;
                            if (rshift > 0) {
                                final_exponent += rshift;
                                int roundup = nil::crypto3::multiprecision::cpp_bf_io_detail::get_round_mode(
                                    t, rshift - 1, error);
                                t >>= rshift;
                            ...
                            if (final_exponent >
                                ...
                                final_exponent -=
                                    cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent;
                            } else if (final_exponent < cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                                                      MaxE>::min_exponent) {
                                // Underflow:
                                exponent() =
                        ...
                        do {
                            cpp_int d;
                            calc_exp = nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_pow(
                                d, cpp_int(5), -decimal_exp, max_bits, error);
                            int shift =
                            ...
                            if (gb ==
                                ...
                                roundup =
                                    nil::crypto3::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, q);
                            } else if (bit_test(q, gb - (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE,
                                                                           MaxE>::bit_count) &&
                                       ((int)lsb(q) == (gb - (int)cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                                                                MinE, MaxE>::bit_count))) {
                ...
                template<unsigned Digits, digit_base_type DigitBase, class Allocator, class Exponent, Exponent MinE,
                         Exponent MaxE>
                std::string cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::str(
                    std::streamsize dig, std::ios_base::fmtflags f) const {
                    if (dig == 0)
                        dig = std::numeric_limits<
                            number<cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>>>::max_digits10;
                    ...
                    if (exponent() <= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent) {
                        ...
                        std::intmax_t shift =
                            (std::intmax_t)
                                cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count -
                            (std::intmax_t)exponent() - 1;
                        std::intmax_t digits_wanted = static_cast<int>(dig);
                        std::intmax_t base10_exp = exponent() >= 0 ?
                                                       static_cast<std::intmax_t>(std::floor(0.30103 * exponent())) :
                                                       static_cast<std::intmax_t>(std::ceil(0.30103 * exponent()));
                        //
                        ...
                        if (digits_wanted < -1) {
                            ...
                            nil::crypto3::multiprecision::detail::format_float_string(s, base10_exp, dig, f, true);
                            return s;
                        }
                        //
                        ...
                        // MaxE>::bit_count to avoid large computations and excessive memory usage, but we also want to
                        // avoid having to up the computation and start again at a higher precision. So we round
                        // cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count up to the
                        // nearest whole number of limbs, and add one limb for good measure.  This works very well for
                        // small exponents, but for larger exponents we add a few extra limbs to max_bits:
                        //
#ifdef BOOST_MP_STRESS_IO
                        std::intmax_t max_bits =
                            cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + 32;
                        ...
                        std::intmax_t max_bits =
                            ...
                            limb_bits;
                        if (power10)
                            max_bits += (msb(nil::crypto3::multiprecision::detail::abs(power10)) / 8) * limb_bits;
#endif
                        do {
                            std::int64_t error = 0;
                            std::intmax_t calc_exp = 0;
                            ...
                            if (shift < 0) {
                                if (power10 >= 0) {
                                    ...
                                    cpp_int d;
                                    calc_exp = nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_pow(
                                        d, cpp_int(5), -power10, max_bits, error);
                                    shift += calc_exp;
                                    ...
                                    divide_qr(i, d, i, r);
                                    roundup =
                                        nil::crypto3::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, i);
                                    if (roundup < 0) {
#ifdef BOOST_MP_STRESS_IO
                                        max_bits += 32;
#else
                                        max_bits *= 2;
#endif
                                        shift = (std::intmax_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent,
                                                                             MinE, MaxE>::bit_count -
                                                exponent() - 1 - power10;
                                        continue;
                                    }
                                ...
                                if (power10 > 0) {
                                    if (power10) {
                                        cpp_int t;
                                        calc_exp = nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_pow(
                                            t, cpp_int(5), power10, max_bits, error);
                                        calc_exp += nil::crypto3::multiprecision::cpp_bf_io_detail::restricted_multiply(
                                            i, i, t, max_bits, error);
                                        shift -= calc_exp;
                                    }
                                    ...
                                    if (shift) {
                                        roundup = nil::crypto3::multiprecision::cpp_bf_io_detail::get_round_mode(
                                            i, shift - 1, error);
                                        if (roundup < 0) {
#ifdef BOOST_MP_STRESS_IO
                                            max_bits += 32;
                        ...
                        if ((roundup == 2) || ((roundup == 1) && ((s[s.size() - 1] - '0') & 1))) {
                            nil::crypto3::multiprecision::detail::round_string_up_at(s, static_cast<int>(s.size() - 1),
                                                                                     base10_exp);
                        }
                        ...
                        nil::crypto3::multiprecision::detail::format_float_string(s, base10_exp, dig, f, false);
                    } else {
                        switch (exponent()) {
                            case exponent_zero:
                                s = sign() ? "-0" : f & std::ios_base::showpos ? "+0" : "0";
                                nil::crypto3::multiprecision::detail::format_float_string(s, 0, dig, f, true);
                                break;
                            case exponent_nan:
                                s = "nan";
                                break;
