...
#define NON_MEMBER_OP4(name, str)                                                                                     \
    ...
    template<class Backend, class T>                                                                                  \
    inline void BOOST_JOIN(eval_, name)(debug_adaptor<Backend> & result, const debug_adaptor<Backend>& a,             \
                                        const debug_adaptor<Backend>& b, const T& c) {                                \
        using default_ops::BOOST_JOIN(eval_, name);                                                                   \
        BOOST_JOIN(eval_, name)                                                                                       \
        (result.value(), unwrap_debug_type(a), unwrap_debug_type(b), unwrap_debug_type(c));                           \
        result.update_view();                                                                                         \
    ...
    template<class Backend>                                                                                           \
    inline void BOOST_JOIN(eval_, name)(debug_adaptor<Backend> & result, const debug_adaptor<Backend>& a,             \
                                        const debug_adaptor<Backend>& b, const debug_adaptor<Backend>& c) {           \
        using default_ops::BOOST_JOIN(eval_, name);                                                                   \
        BOOST_JOIN(eval_, name)                                                                                       \
        (result.value(), unwrap_debug_type(a), unwrap_debug_type(b), unwrap_debug_type(c));                           \
        result.update_view();                                                                                         \
    ...
    inline void BOOST_JOIN(eval_, name)(debug_adaptor<Backend> & result, const debug_adaptor<Backend>& a, const T& b, \
        ...
        result.update_view();                                                                                         \
                ...
                template<class Backend>
                inline void eval_left_shift(debug_adaptor<Backend>& arg, const debug_adaptor<Backend>& a,
                                            std::size_t b) {
                    using default_ops::eval_left_shift;
                ...
                template<class Backend>
                inline void eval_right_shift(debug_adaptor<Backend>& arg, const debug_adaptor<Backend>& a,
                                             std::size_t b) {
                    using default_ops::eval_right_shift;
...
namespace std {
    ...
    class numeric_limits<nil::crypto3::multiprecision::number<
        nil::crypto3::multiprecision::backends::debug_adaptor<Backend>, ExpressionTemplates>>
        ...
        using number_type =
            nil::crypto3::multiprecision::number<nil::crypto3::multiprecision::backends::debug_adaptor<Backend>,
                                                 ExpressionTemplates>;
...
namespace boost {
    namespace math {
        ...
        namespace policies {
            ...
            template<class Backend, nil::crypto3::multiprecision::expression_template_option ExpressionTemplates,
                     class Policy>
            struct precision<nil::crypto3::multiprecision::number<nil::crypto3::multiprecision::debug_adaptor<Backend>,
                                                                  ExpressionTemplates>,
                             Policy>
