...
#pragma warning(disable : 4702)    // Unreachable code (reachability depends on template params)
                ...
                template<unsigned MinBits = 0,
                         unsigned MaxBits = 0,
                         nil::crypto3::multiprecision::cpp_integer_type SignType = signed_magnitude,
                ...
                struct is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>> {
                    ...
                    static constexpr const bool value =
                        ...
                        (max_precision<self>::value <=
                         (sizeof(double_limb_type) * CHAR_BIT) - (SignType == signed_packed ? 1 : 0));
                };
            ...
            namespace backends {
                ...
                public:
                    ...
                    static constexpr unsigned internal_limb_count = MinBits ? (MinBits / limb_bits +
                                                                               ((MinBits % limb_bits) ? 1 : 0)) :
                                                                    (sizeof(limb_data) / sizeof(limb_type)) > 1 ?
                ...
                private:
                    union data_type {
                        ...
                        constexpr data_type(signed_limb_type i) noexcept :
                            first(i < 0 ?
                                      static_cast<limb_type>(nil::crypto3::multiprecision::detail::unsigned_abs(i)) :
                                      i) {
                        }
                        ...
                        constexpr data_type(signed_double_limb_type i) noexcept :
                            double_first(i < 0 ? static_cast<double_limb_type>(
                ...
                public:
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(limb_type i) noexcept :
                        m_data(i), m_limbs(1), m_sign(false), m_internal(true), m_alias(false) {
                    }
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(signed_double_limb_type i) noexcept :
                        m_data(i),
                        m_limbs(i < 0 ?
                                    (static_cast<double_limb_type>(nil::crypto3::multiprecision::detail::unsigned_abs(
                                         i)) > static_cast<double_limb_type>(max_limb_value) ?
                                         2 :
                                         1) :
                                    (i > max_limb_value ? 2 : 1)),
                        m_sign(i < 0), m_internal(true), m_alias(false) {
                    }
#endif
                    //
                    ...
                    public:
                        scoped_shared_storage(const allocator_type& a, unsigned len) :
                            boost::empty_value<allocator_type>(boost::empty_init_t(), a), capacity(len), allocated(0),
                            is_alias(false) {
                            data = allocator().allocate(len);
                        }
                        scoped_shared_storage(const cpp_int_base& i, unsigned len) :
                            boost::empty_value<allocator_type>(boost::empty_init_t(), i.allocator()), capacity(len),
                            allocated(0), is_alias(false) {
                            data = allocator().allocate(len);
                        }
                        scoped_shared_storage(limb_type* limbs, unsigned n) :
                            data(limbs), capacity(n), allocated(0), is_alias(true) {
                        }
                    ...
                    explicit constexpr cpp_int_base(limb_type* data, unsigned offset, unsigned len) noexcept :
                        m_data(data + offset, len), m_limbs(len), m_sign(false), m_internal(false), m_alias(true) {
                    }
                    ...
                    explicit constexpr cpp_int_base(const limb_type* data, unsigned offset, unsigned len) noexcept :
                        m_data(const_cast<limb_type*>(data) + offset, len), m_limbs(len), m_sign(false),
                        m_internal(false), m_alias(true) {
                    }
                    explicit cpp_int_base(scoped_shared_storage& data, unsigned len) noexcept :
                        m_data(data.allocate(len), len), m_limbs(len), m_sign(false), m_internal(false), m_alias(true) {
                    }
                    //
                    // Helper functions for getting at our internal data, and manipulating storage:
                    //
                    BOOST_MP_FORCEINLINE allocator_type& allocator() noexcept {
                        return base_type::get();
                    }
                    BOOST_MP_FORCEINLINE const allocator_type& allocator() const noexcept {
                        return base_type::get();
                    }
                    BOOST_MP_FORCEINLINE unsigned size() const noexcept {
                        return m_limbs;
                    }
                    BOOST_MP_FORCEINLINE limb_pointer limbs() noexcept {
                        return m_internal ? m_data.la : m_data.ld.data;
                    }
                    BOOST_MP_FORCEINLINE const_limb_pointer limbs() const noexcept {
                        return m_internal ? m_data.la : m_data.ld.data;
                    }
                    BOOST_MP_FORCEINLINE unsigned capacity() const noexcept {
                        return m_internal ? internal_limb_count : m_data.ld.capacity;
                    }
                    BOOST_MP_FORCEINLINE bool sign() const noexcept {
                        return m_sign;
                    }
                    ...
                    void resize(unsigned new_size, unsigned min_size) {
                        constexpr const unsigned max_limbs = MaxBits / (CHAR_BIT * sizeof(limb_type)) +
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base() noexcept :
                        m_data(), m_limbs(1), m_sign(false), m_internal(true), m_alias(false) {
                    }
                    BOOST_MP_FORCEINLINE cpp_int_base(const cpp_int_base& o) :
                        base_type(o), m_limbs(o.m_alias ? o.m_limbs : 0), m_sign(o.m_sign),
                        m_internal(o.m_alias ? false : true), m_alias(o.m_alias) {
                        if (m_alias) {
                    ...
                    cpp_int_base& operator=(
                        cpp_int_base<MinBits2, MaxBits2, signed_magnitude, Checked2, Allocator>&& o) noexcept {
                        if (o.m_internal) {
                            m_sign = o.m_sign;
                        ...
                        if (!m_internal && !m_alias)
                            allocator().deallocate(m_data.ld.data, m_data.ld.capacity);
                        *static_cast<base_type*>(this) = static_cast<
                            typename cpp_int_base<MinBits2, MaxBits2, signed_magnitude, Checked2, Allocator>::
                                base_type&&>(o);
                        m_limbs = o.m_limbs;
                        m_sign = o.m_sign;
                        m_internal = o.m_internal;
                ...
                template<unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
                const limb_type
                    cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::max_limb_value;
                template<unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
                const limb_type
                    cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::sign_bit_mask;
                template<unsigned MinBits, unsigned MaxBits, cpp_int_check_type Checked, class Allocator>
                const unsigned
                    cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>::internal_limb_count;
                ...
                struct cpp_int_base<MinBits, MaxBits, unsigned_magnitude, Checked, Allocator, false>
                    ...
                    //
                    static_assert(((sizeof(Allocator) == 0) && !std::is_void<Allocator>::value),
                ...
                struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false> {
                    ...
                    // Interface invariants:
                    //
                    static_assert(MinBits > sizeof(double_limb_type) * CHAR_BIT,
                                  "Template parameter MinBits is inconsistent with the parameter trivial - did you "
                ...
                public:
                    ...
                    static constexpr limb_type upper_limb_mask =
                        (MinBits % limb_bits) ? (limb_type(1) << (MinBits % limb_bits)) - 1 : (~limb_type(0));
...
// if we left both constructors, 'ambiguous overloading' error occurs.
                ...
                public:
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(signed_limb_type i) noexcept :
                        m_wrapper(
                            limb_type(i < 0 ? static_cast<limb_type>(-static_cast<signed_double_limb_type>(i)) : i)),
                        m_limbs(1), m_sign(i < 0) {
                    }
#if BOOST_ENDIAN_LITTLE_BYTE && !defined(BOOST_MP_TEST_NO_LE)
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(double_limb_type i) noexcept :
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(signed_double_limb_type i) noexcept :
                        m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(
                                                               nil::crypto3::multiprecision::detail::unsigned_abs(i)) :
                                                           i)),
                        m_limbs(i < 0 ? (static_cast<double_limb_type>(
                                             nil::crypto3::multiprecision::detail::unsigned_abs(i)) > max_limb_value ?
                                             2 :
                                             1) :
                                        (i > max_limb_value ? 2 : 1)),
                        m_sign(i < 0) {
                    ...
                    constexpr cpp_int_base(literals::detail::value_pack<> i) : m_wrapper(i), m_limbs(1), m_sign(false) {
                    }
                    ...
                    explicit constexpr cpp_int_base(scoped_shared_storage&, unsigned) noexcept :
                        m_wrapper(), m_limbs(0), m_sign(false) {
                    }
                    //
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
                        resize(unsigned new_size, unsigned min_size) noexcept((Checked == unchecked)) {
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void normalize() noexcept((Checked == unchecked)) {
                        limb_pointer p = limbs();
                        detail::verify_limb_mask(
                            m_limbs == internal_limb_count, p[m_limbs - 1], upper_limb_mask, checked_type());
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base() noexcept :
                        m_wrapper(limb_type(0u)), m_limbs(1), m_sign(false) {
                    }
                ...
                template<unsigned MinBits, cpp_int_check_type Checked>
                const unsigned
                    cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>::internal_limb_count;
                //
                ...
                public:
                    ...
                    // Direct construction:
                    //
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(limb_type i) noexcept : m_wrapper(i), m_limbs(1) {
                    }
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR
                        cpp_int_base(signed_limb_type i) noexcept((Checked == unchecked)) :
                        m_wrapper(
                            limb_type(i < 0 ? static_cast<limb_type>(-static_cast<signed_double_limb_type>(i)) : i)),
                        m_limbs(1) {
                        if (i < 0)
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR
                        cpp_int_base(signed_double_limb_type i) noexcept((Checked == unchecked)) :
                        m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(
                                                               nil::crypto3::multiprecision::detail::unsigned_abs(i)) :
                                                           i)),
                        m_limbs(i < 0 ? (static_cast<double_limb_type>(
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base() noexcept : m_wrapper(limb_type(0u)), m_limbs(1) {
                    }
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(const cpp_int_base& o) noexcept :
                ...
                template<unsigned MinBits, cpp_int_check_type Checked>
                const limb_type
                    cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::max_limb_value;
                template<unsigned MinBits, cpp_int_check_type Checked>
                const limb_type cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::sign_bit_mask;
                template<unsigned MinBits, cpp_int_check_type Checked>
                const unsigned
                    cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>::internal_limb_count;
                //
                ...
                protected:
                    ...
                    static constexpr local_limb_type limb_mask =
                        (MinBits < limb_bits) ?
                            local_limb_type((local_limb_type(~local_limb_type(0))) >> (limb_bits - MinBits)) :
                            local_limb_type(~local_limb_type(0));
                ...
                protected:
                    ...
                    BOOST_MP_CXX14_CONSTEXPR
                        typename std::enable_if<!(!nil::crypto3::multiprecision::detail::is_integral<T>::value ||
                                                  (std::numeric_limits<T>::is_specialized &&
                                                   (std::numeric_limits<T>::digits <= (int)MinBits)))>::type
                        check_in_range(T val, const std::integral_constant<int, checked>&) {
                        using common_type = typename std::common_type<
                            typename nil::crypto3::multiprecision::detail::make_unsigned<T>::type,
                            local_limb_type>::type;

                        if (static_cast<common_type>(nil::crypto3::multiprecision::detail::unsigned_abs(val)) >
                            static_cast<common_type>(limb_mask))
                    ...
                    template<class T>
                    BOOST_MP_CXX14_CONSTEXPR void check_in_range(T val) noexcept(
                        noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<T>(), checked_type()))) {
                ...
                public:
                    ...
                    //
                    template<class SI>
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(
                        SI i,
                        typename std::enable_if<nil::crypto3::multiprecision::detail::is_signed<SI>::value &&
                            ...
                            0) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>()))) :
                        m_data(
                            i < 0 ?
                                static_cast<local_limb_type>(
                                    static_cast<typename nil::crypto3::multiprecision::detail::make_unsigned<SI>::type>(
                                        nil::crypto3::multiprecision::detail::unsigned_abs(i)) &
                                    limb_mask) :
                    ...
                    template<class SI>
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(
                        SI i,
                        typename std::enable_if<nil::crypto3::multiprecision::detail::is_signed<SI>::value &&
                                                nil::crypto3::multiprecision::detail::is_integral<SI>::value &&
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(
                        ...
                        typename std::enable_if<nil::crypto3::multiprecision::detail::is_unsigned<UI>::value &&
                                                (Checked == checked)>::type const* =
                            0) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<UI>()))) :
                        m_data(static_cast<local_limb_type>(i) & limb_mask),
                        m_sign(false) {
                        check_in_range(i);
                    }
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(
                        F i,
                        typename std::enable_if<std::is_floating_point<F>::value && (Checked == checked)>::type const* =
                            0) :
                        m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask),
                        m_sign(i < 0) {
                        check_in_range(i);
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR bool sign() const noexcept {
                        return m_sign;
                    }
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void sign(bool b) noexcept {
                        m_sign = b;
                    ...
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void resize(unsigned /* new_size */,
                                                                              unsigned min_size) {
                ...
                protected:
                    ...
                    template<class T>
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void
                        check_in_range(T val) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(
                            std::declval<T>(),
                            checked_type(),
                            nil::crypto3::multiprecision::detail::is_signed<T>()))) {
                        check_in_range(val, checked_type(), nil::crypto3::multiprecision::detail::is_signed<T>());
                    }
                ...
                public:
                    ...
                    BOOST_MP_FORCEINLINE constexpr cpp_int_base(const cpp_int_base& o) noexcept : m_data(o.m_data) {
                    }
                    //~cpp_int_base() noexcept {}
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void assign(const cpp_int_base& o) noexcept {
                        m_data = o.m_data;
                ...
                struct is_allowed_cpp_int_base_conversion
                    : public std::conditional<std::is_same<Arg, limb_type>::value ||
                                                  ...
                                                  (is_trivial_cpp_int<Base>::value &&
                                                   nil::crypto3::multiprecision::detail::is_arithmetic<Arg>::value),
    ...
    !defined(BOOST_NO_CXX14_CONSTEXPR)
                ...
                private:
                    ...
                    template<class A>
                    BOOST_MP_CXX14_CONSTEXPR
                        typename std::enable_if<!nil::crypto3::multiprecision::detail::is_integral<A>::value>::type
                        do_assign_arithmetic(A val, const std::integral_constant<bool, true>&) {
                        this->check_in_range(val);
                        *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(
                                                         nil::crypto3::multiprecision::detail::abs(val)) :
                                                     static_cast<typename self_type::local_limb_type>(val);
                        this->sign(val < 0);
                        this->normalize();
                    }
                    ...
                    BOOST_MP_CXX14_CONSTEXPR void do_assign_arithmetic(
                        ...
                        if (i < 0)
                            s = true;
                        double_limb_type ui =
                            static_cast<double_limb_type>(nil::crypto3::multiprecision::detail::unsigned_abs(i));
                        typename base_type::limb_pointer p = this->limbs();
#ifdef __MSVC_RUNTIME_CHECKS
                ...
                public:
                    ...
                    template<class Arithmetic>
                    BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR
                        typename std::enable_if<nil::crypto3::multiprecision::detail::is_arithmetic<Arithmetic>::value,
                                                int>::type
...
//
#include <nil/crypto3/multiprecision/cpp_int/limits.hpp>
#include <nil/crypto3/multiprecision/cpp_int/comparison.hpp>
#include <nil/crypto3/multiprecision/cpp_int/add.hpp>
#include <nil/crypto3/multiprecision/cpp_int/multiply.hpp>
