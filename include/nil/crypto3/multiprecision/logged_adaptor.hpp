    ...
    namespace crypto3 {
        namespace multiprecision {
            ...
            namespace backends {
                ...
                template<class T>
                inline const T& unwrap_logged_type(const T& a) {
                    return a;
                }
                template<class Backend>
...
#define NON_MEMBER_OP4(name, str)                                                                                   \
    ...
    inline void BOOST_JOIN(eval_, name)(logged_adaptor<Backend> & result, const logged_adaptor<Backend>& a,         \
        ...
        log_postfix_event(result.value(), str);                                                                     \
                ...
                template<class Backend>
                inline void eval_left_shift(logged_adaptor<Backend>& arg, const logged_adaptor<Backend>& a,
                                            std::size_t b) {
                    using default_ops::eval_left_shift;
                ...
                template<class Backend>
                inline void eval_right_shift(logged_adaptor<Backend>& arg, const logged_adaptor<Backend>& a,
                                             std::size_t b) {
                    using default_ops::eval_right_shift;
                    log_prefix_event(arg.value(), a, b, ">>");
